...
try:
    from . import vast_util
    from .vast_util import *
except ImportError:
    import vast_util         # type: ignore
    from vast_util import *  # type: ignore
...
@parser.command(
    argument("-g", "--gpu-name", type=str, required=True, choices=get_gpu_names(), help="Name of the GPU model, replace spaces with underscores"),
...
def launch__instance(args):
    ...
    if args.region:
        if not is_valid_region(args.region):
            print("Invalid region or country codes provided.")
            return
        region_query = parse_region(args.region)
...
@parser.command(
    ...
    argument("--cols",              metavar="COLS",       help=f"override displayed columns with a comma-separated list (available: {','.join(s[0] for s in INSTANCE_COL_SPECS)})"),
...
def show__instances_v1(args):
    ...
    if args.status:
        invalid_statuses = [s for s in args.status if s not in STATUS_COLORS]
        if invalid_statuses:
            valid = ", ".join(sorted(STATUS_COLORS))
    ...
    if not args.raw:
        params["select_cols"] = VERBOSE_INSTANCE_SELECT_COLS if args.verbose else DEFAULT_INSTANCE_SELECT_COLS
    if not has_rich:
    ...
    while looping:
        ...
        if has_rich:
            if page == 1 or args.all:
                if filter_combos:
                    output_parts.append(rich_object_to_string(build_filters_panel(filter_combos), no_color=args.no_color))
                output_parts.append(rich_object_to_string(build_summary_panel(
                    total, label_cnts,
            ...
            if not instances:
                ...
                tbl, hidden = build_instances_table(instances, verbose=args.verbose, cols=user_cols)
